...
class RandomMixing(nn.Module):
    def __init__(self, num_tokens=1200, **kwargs):
        super().__init__()
...
class MetaFormer(nn.Module):
    ...
    def forward_features(self, x):
        ...
        for i in range(self.num_stage):
            norm = self.output_norm(self.dims[i]).cuda()
...
@register_model
def identityformer_s12(depths,dims,pretrained=False, **kwargs):
    model = MetaFormer(
        depths=depths,
        dims=dims,
        token_mixers=nn.Identity,
...
@register_model
def convformer_s18(depths,dims,pretrained=False, **kwargs):
    model = MetaFormer(
        depths=depths,
        dims=dims,
...
@register_model
def caformer_s18(depths,dims,pretrained=False, **kwargs):
    model = MetaFormer(
        depths=depths,
        dims=dims,
...
@register_model
def caformer_b36_in21k(depths,dims,pretrained=False, **kwargs):
    model = MetaFormer(
        depths=depths,
        dims=dims,
        token_mixers=[SepConv, SepConv, Attention, Attention],
...
@register_model
def randformer_s12(depths,dims,pretrained=False, **kwargs):
    model = MetaFormer(
        depths=depths,
        dims=dims,
        token_mixers=[nn.Identity, nn.Identity, RandomMixing, partial(RandomMixing, num_tokens=300)],
        norm_layers=partial(LayerNormGeneral, normalized_dim=(1, 2, 3), eps=1e-6, bias=False),
        **kwargs)
    return model