...
from PixelFormer.networks.net_utils import load_checkpoint
...
class SwinMoHWindowAttention(nn.Module):
    ...
    """
    SWIN_LOAD_BALANCING_LOSSES = []
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0., shared_head=0, routed_head=0):
        super().__init__()
        self.dim = dim
    ...
    def forward(self, x, mask=None):
        ...
        if self.routed_head > 0:
            ...
            if self.training:
                ...
                SwinMoHWindowAttention.SWIN_LOAD_BALANCING_LOSSES.append(l_aux)
        ...
        if self.routed_head > 0:
            ...
            if self.shared_head > 1:
                ...
                shared_head_gates = torch.ones((B_, N, self.shared_head)).to(_x.device).to(_x.dtype) * self.shared_head
...
class SwinTransformerBlock(nn.Module):
    ...
    def __init__(self, dim, num_heads, shared_head, routed_head, window_size=7, shift_size=0,
        ...
        self.attn = SwinMoHWindowAttention(
            dim, window_size=to_2tuple(self.window_size), num_heads=num_heads, shared_head=shared_head, routed_head=routed_head,
...
class MoHSwinTransformer(nn.Module):
    ...
    def __init__(self,
        ...
        for i_layer in range(self.num_layers):
            layer = BasicLayer(
                ...
                shared_head=shared_head[i_layer],
                routed_head=routed_head[i_layer],
                window_size=window_size,
                mlp_ratio=mlp_ratio,
                qkv_bias=qkv_bias,
                qk_scale=qk_scale,
    ...
    def train(self, mode=True):
        ...
        super(MoHSwinTransformer, self).train(mode)
        self._freeze_stages()