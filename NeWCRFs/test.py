...
from NeWCRFs.datasets.dataloader import *
...
def test(args):
    ...
    prep_str = formatter.format(
        f"Preparing testing of NeWCRFs {args.encoder[:-2]}", 
        color = 'blue', 
        style = ['bold'], 
        separator = True
    )
    ...
    if args.opt_type != "":
        ...
        full_str = f"Starting {opt_name} {where} NeWCRFs {args.encoder[:-2]} testing"
    else:
        opt_name = ""
        where = ""
        full_str = f"Starting NeWCRFs {args.encoder[:-2]} testing"
