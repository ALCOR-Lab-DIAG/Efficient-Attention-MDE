...
from NeWCRFs.networks.meta_newcrf_layers import MetaNewCRF
from NeWCRFs.networks.moh_newcrf_layers import MoHNewCRF
from NeWCRFs.networks.pyra_newcrf_layers import PyraNewCRF
from NeWCRFs.networks.meta_swin_transformer import MetaSwinTransformer
from NeWCRFs.networks.moh_swin_transformer import MoHSwinTransformer
from NeWCRFs.networks.pyra_swin_transformer import PyraSwinTransformer
...
class NewCRFDepth(nn.Module):
    ...
    def __init__(self, version=None, inv_depth=False, pretrained=None, 
        ...
        if self.opt_type == "meta" and (self.opt_where == "dec" or self.opt_where == "full"):
            ...
            self.crf3 = MetaNewCRF(input_dim=in_channels[3], embed_dim=crf_dims[3], window_size=win, v_dim=v_dims[3], num_heads=32)
            self.crf2 = MetaNewCRF(input_dim=in_channels[2], embed_dim=crf_dims[2], window_size=win, v_dim=v_dims[2], num_heads=16)
            self.crf1 = MetaNewCRF(input_dim=in_channels[1], embed_dim=crf_dims[1], window_size=win, v_dim=v_dims[1], num_heads=8)
            self.crf0 = MetaNewCRF(input_dim=in_channels[0], embed_dim=crf_dims[0], window_size=win, v_dim=v_dims[0], num_heads=4)
        elif self.opt_type == "pyra" and (self.opt_where == "dec" or self.opt_where == "full"):
            print("[LOG] Building PYRA-SAM decoder")
            self.crf3 = PyraNewCRF(input_dim=in_channels[3], embed_dim=crf_dims[3], window_size=win, v_dim=v_dims[3], num_heads=32)
            self.crf2 = PyraNewCRF(input_dim=in_channels[2], embed_dim=crf_dims[2], window_size=win, v_dim=v_dims[2], num_heads=16)
            self.crf1 = PyraNewCRF(input_dim=in_channels[1], embed_dim=crf_dims[1], window_size=win, v_dim=v_dims[1], num_heads=8)
            self.crf0 = PyraNewCRF(input_dim=in_channels[0], embed_dim=crf_dims[0], window_size=win, v_dim=v_dims[0], num_heads=4)
        elif self.opt_type == "moh" and (self.opt_where == "dec" or self.opt_where == "full"):
            print("[LOG] Building MoH-SAM decoder")
            self.crf3 = MoHNewCRF(input_dim=in_channels[3], embed_dim=crf_dims[3], window_size=win, v_dim=v_dims[3], num_heads=32, shared_head=5, routed_head=16)
            self.crf2 = MoHNewCRF(input_dim=in_channels[2], embed_dim=crf_dims[2], window_size=win, v_dim=v_dims[2], num_heads=16, shared_head=4, routed_head=8)
            self.crf1 = MoHNewCRF(input_dim=in_channels[1], embed_dim=crf_dims[1], window_size=win, v_dim=v_dims[1], num_heads=8, shared_head=8, routed_head=0)
            self.crf0 = MoHNewCRF(input_dim=in_channels[0], embed_dim=crf_dims[0], window_size=win, v_dim=v_dims[0], num_heads=4, shared_head=4, routed_head=0)
        else:
