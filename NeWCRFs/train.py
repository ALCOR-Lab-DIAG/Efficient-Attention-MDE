...
from NeWCRFs.datasets.dataloader import NewDataLoader
from NeWCRFs.globals import *
from NeWCRFs.networks.NewCRFDepth import NewCRFDepth
from NeWCRFs.networks.moh_swin_transformer import SwinMoHWindowAttention
from NeWCRFs.networks.moh_newcrf_layers import NeWCFRsMoHWindowAttention
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    # Build the model
    model_name = "NewCRFs " + args.encoder[:-2]
    ...
    print(f"[Model] NeWCRFs version: {args.encoder[:-2]}")
    ...
    print(f"[Output] NeWCRFs depth shape: {tuple(dummy_depth.squeeze(0).shape)}")
    ...
    if args.opt_type is not None:
        ...
        full_str = f"Starting {opt_name} {where} NeWCRFs {args.encoder[:-2]} training"
    else:
        opt_name = ""
        where = ""
        full_str = f"Starting NeWCRFs {args.encoder[:-2]} training"
        ...
        for step, sample_batched in enumerate(dataloader.data):
            if args.opt_type == "moh":
                if args.opt_where == "enc":
                    ...
                    NeWCFRsMoHWindowAttention.NeWCFRs_LOAD_BALANCING_LOSSES.clear()
                else:
            ...
            if args.opt_type == "moh":
                ...
                if args.opt_where == "enc":
                    ...
                    moh_losses_on_device = [tensor.to(args.gpu) for tensor in NeWCFRsMoHWindowAttention.NeWCFRs_LOAD_BALANCING_LOSSES]
                else:
                    # Unisci entrambe le liste
                    moh_losses_on_device = (
                        [tensor.to(args.gpu) for tensor in SwinMoHWindowAttention.SWIN_LOAD_BALANCING_LOSSES] +
                        [tensor.to(args.gpu) for tensor in NeWCFRsMoHWindowAttention.NeWCFRs_LOAD_BALANCING_LOSSES]
                    )
    ...
    send_email("NeWCRFs", version, args.opt_type, args.opt_where)

def train(args):
    formatter = TextFormatter()
    prep_str = formatter.format(
        f"Preparing training of NeWCRFs {args.encoder[:-2]}", 
        color = 'blue', 
        style = ['bold'], 
        separator = True
    )
