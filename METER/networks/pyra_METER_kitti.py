...
class Attention(nn.Module):
    ...
    def forward(self, x):
        ...
        q, _, _ = map(lambda t: rearrange(t, 'b p n (h d) -> b p h n d', h=self.heads), qkv)
#########################################################
        _, _, _, C = x.shape
        ...
        qkv_ = self.to_qkv(x_)
        # print('----qkv_ shape: ', qkv_.shape)
        qkv_ = qkv_.chunk(3, dim=-1)
        _, k, v = map(lambda t: rearrange(t, 'b p n (h d) -> b p h n d', h=self.heads), qkv_)

        if (k.shape[3] & self.sr_ratio == 0):
            k = k.reshape(k.shape[0], k.shape[1]*self.sr_ratio, k.shape[2], k.shape[3]//self.sr_ratio, k.shape[4])
            v = v.reshape(v.shape[0], v.shape[1]*self.sr_ratio, v.shape[2], v.shape[3]//self.sr_ratio, v.shape[4])
        else:
            _, k, v = map(lambda t: rearrange(t, 'b p n (h d) -> b p h n d', h=self.heads), qkv)
#########################################################
