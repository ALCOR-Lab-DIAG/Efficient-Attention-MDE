...
class Attention(nn.Module):
    ...
    def __init__(self, dim, heads=8, dim_head=64, dropout=0.,shared_head=2, routed_head=2):
        super().__init__()
        ...
        if self.routed_head > 0:
            self.wg = torch.nn.Linear(dim, heads - shared_head, bias=False)
    ...
    def forward(self, x):
        ...
        q, k, v = map(lambda t: rearrange(t, 'p n (h d) -> p h n d', h=self.heads), qkv)
...
# class Attention(nn.Module):
#     LOAD_BALANCING_LOSSES = []
#     def __init__(self, dim, heads=8, dim_head=64, dropout=0., shared_head=1, routed_head=3):
#         super().__init__()
#         inner_dim = dim_head * heads
#         project_out = not (heads == 1 and dim_head == dim)
...
#         # Routing mechanism similar to SwinMoHWindowAttention
#         if self.routed_head > 0:
#             self.wg = nn.Linear(dim, heads - shared_head, bias=False)
#             if self.shared_head > 0:
...
#     def forward(self, x):
#         # Adjust for potential 5D input
#         if x.dim() == 5:
#             B, H, N, E, C = x.shape
#             x = x.reshape(B, H, N, E * C)
#         else:
#             B, H, N, C = x.shape

#         # Flatten for routing
#         _x = x.reshape(B * H * N, C)
...
#             num_tokens, num_experts = gates.shape
#             _, indices = torch.topk(gates, k=self.routed_head, dim=1)
#             moh_mask = F.one_hot(indices, num_classes=num_experts).sum(dim=1)

#             # Load balancing loss (optional)
#             if self.training:
#                 me = gates.mean(dim=0)
#                 ce = moh_mask.float().mean(dim=0)
#                 l_aux = torch.mean(me * ce) * num_experts * num_experts
#                 print(l_aux)
#                 Attention.LOAD_BALANCING_LOSSES.append(l_aux)

#             # Normalize gates
#             routed_head_gates = gates * moh_mask
#             denom_s = torch.sum(routed_head_gates, dim=1, keepdim=True)
#             denom_s = torch.clamp(denom_s, min=torch.finfo(denom_s.dtype).eps)
#             routed_head_gates /= denom_s
#             routed_head_gates = routed_head_gates.reshape(B, H, N, -1)

#         # Standard QKV computation
#         qkv = self.to_qkv(x).chunk(3, dim=-1)
        
#         # Adjust rearrange to handle potential 5D input
#         q, k, v = map(lambda t: t.reshape(B, H, N, self.heads, -1).transpose(3, 2), qkv)
...
#         out = out.transpose(2, 3).reshape(B, H, N, -1)
        
#         # Handle routed heads if enabled
#         if self.routed_head > 0:
#             # Handle shared heads
#             if self.shared_head > 1:
#                 shared_head_weight = self.wg_1(_x)
#                 shared_head_gates = F.softmax(shared_head_weight, dim=1).reshape(B, H, N, -1)
#             else:
#                 shared_head_gates = torch.ones((B, H, N, self.shared_head)).to(_x.device).to(_x.dtype)
            
#             # Mixing shared and routed heads
#             if self.shared_head == 0:
#                 masked_gates = routed_head_gates
#             else:
#                 weight_0 = self.wg_0(_x)
#                 weight_0 = F.softmax(weight_0, dim=1).reshape(B, H, N, 2)
                
#                 shared_head_gates = shared_head_gates * weight_0[:,:,:,0:1]
#                 routed_head_gates = routed_head_gates * weight_0[:,:,:,1:2]

#                 masked_gates = torch.cat([shared_head_gates, routed_head_gates], dim=3)

#             # Reshape gates to match out dimensions
#             masked_gates = masked_gates.unsqueeze(-1)
#             out = out.unsqueeze(3)  # Add an extra dimension
            
#             # Perform element-wise multiplication
#             out = masked_gates * out
#             out = out[:,:,:,0,:]   # Remove the extra dimension
